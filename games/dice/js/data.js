            ...
            if (arGame.length > 10) {
                for (var i = arGame.length - 10; i <= arGame.length - 1; i++) {
                    if (arGame[i].length > 5) {
                        ...
                        $(".dice-table").prepend('<tr><td><a target="_blank" href="https://testnet.etherscan.io/address/0x'+time.substr(2)+' "> ' + time.slice(2, 12) + '...</a> </td><td>' + chance + ' %</td><td>' + result + '</td><td>' + bet + ' ETH</td><td>x'  + (99/chance).toFixed(3) + '</td><td>' + profit.toFixed(3) + '</td></tr>');
                    }
                }
            }
        }
