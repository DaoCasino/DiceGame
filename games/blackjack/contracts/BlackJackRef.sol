...
contract BlackJack is owned {
    ...
    private
  {
    ...
    if (player) {
      ...
      storageGame.playerScore = getPoints(cards);
    } else {
      ...
      storageGame.houseScore = getPoints(cards);
    }
  ...
  function hit() public  {
    ...
    if (games[msg.sender].state != GameState.InProgress) { // the game finished
      if (splitGames[msg.sender].state == GameState.InProgressSplit) { // there was a split game
        ...
        while (games[msg.sender].houseScore < 17) {
          dealCard(false, games[msg.sender]);
        }
  ...
  function checkGameResult(Game storage game, bool finishGame, bool split) private {
    ...
    GameStatus(game.houseScore, game.houseCards.length, game.playerScore, game.playerCards.length);
    ...
    returns (uint8)
  {
    ...
    function getPoints(uint8[] cards) private returns (uint8){
	    uint8 myPoints = 0;
	    uint8 countAce = 0;
	    uint8 i = 0;
    	while(i < cards.length){
    	    uint8 newCard = cards[i];
    		uint8 curPoint = Deck.valueOf(newCard, true);
    		myPoints += curPoint;
    		if(curPoint == 11){
    			countAce ++;
    		}
